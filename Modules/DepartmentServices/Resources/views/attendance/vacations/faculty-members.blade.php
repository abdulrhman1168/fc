                ...
                <tr>
                    ...
                    <td>
                        @if($vacation->status == 6)
                            {!! Form::model($vacation->id, ['method' => 'post', 'route' => ['confirm-faculty-members-vacation', $vacation->id], 'class' =>'form-inline yes-no-submit-modal-form', 'style' => 'display: inline;']) !!}
                                {!! Form::hidden('id', $vacation->id) !!}
                                {!! Form::hidden('status', 9) !!}
                                <button type="submit" class="btn btn-success yes-no-submit-button-form" >
                                <i class="fa fa-check"></i>
                                {{ __('departmentservices::app.agree') }}
                                </button>
                            {!! Form::close() !!}
                            {!! Form::model($vacation->id, ['method' => 'post', 'route' => ['confirm-faculty-members-vacation', $vacation->id], 'class' =>'form-inline yes-no-submit-modal-form', 'style' => 'display: inline;']) !!}
                                {!! Form::hidden('id', $vacation->id) !!}
