...
set('repository', 'git@172.16.19.53:swunit/TransportSystem.git');
set('ssh_type', 'native');
set('ssh_multiplexing', true);

// Servers

server('staging', '172.16.19.72')
    ->stage('staging')
    ->set('branch', 'staging')
    ->set('ssh_type', 'native')
    ->user('eservices')
    ->identityFile('~/.ssh/deploy_id_rsa_2.pub', '~/.ssh/deploy_id_rsa_2')
    ->set('deploy_path', '/home/eservices/staging/backend/tr')
    ->pty(true);


server('production', '172.16.19.50')
    ->stage('production')
    ->set('branch', 'master')
    ->set('ssh_type', 'native')
    ->user('eservices')
    ->identityFile('~/.ssh/deploy_id_rsa_2.pub', '~/.ssh/deploy_id_rsa_2')
    ->set('deploy_path', '/home/eservices/production/backend/tr')
    ->pty(true);
