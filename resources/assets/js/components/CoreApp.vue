...
<li>
  <div :class="{bold: isFolder}" @dblclick="changeType">
    ...
    <span v-show="isArLang" @click="selectApp(model)" :class="{'selected-core-app': isSelected }"> {{model.name}} </span>
    <span v-show="isEngLang" @click="selectApp(model)" :class="{'selected-core-app': isSelected }"> {{model.name_en}} </span>
    ...
    <div v-show="displayPermissionMenu" class="core-app-selected-actions">
      ...
      <span v-show="currentPermission" class="fa fa-pencil core-app-selected-actions-edit" @click="editPermission(currentPermission)"></span>
  ...
  <ul v-show="open" v-if="isFolder">
    <core-app class="core-app-item"
               v-for="model in model.children_recursive"
              :model="model"
              :key="model.id"
...
export default {
  ...
  computed: {
    ...
    isFolder() {
      return this.model.children_recursive &&
        this.model.children_recursive.length
    },
    isArLang() {
       return (this.trans('messages.lang') == "ar");
    },
    isEngLang() {
      return (this.trans('messages.lang') == "en");
    }
  },
  methods: {
    toggle() {
    ...
    editPermission(permission) {
      var data =  { permissionableType: this.permissionableType, permissionableId: this.permissionableId, permission: permission };
      VueEventHandler.fire('EditPermission', data);
    },
