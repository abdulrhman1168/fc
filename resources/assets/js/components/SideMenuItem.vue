...
<li class="nav-item nav-menu-item-class" v-bind:class="{ active: isActive }" :id="itemId">
  <a v-if="hasChildren" href="javascript:;" class="nav-link nav-toggle">
      <i :class="model.icon"></i>
        <span class="title" v-show="isArLang">{{ model.name }}</span>
        <span class="title" v-show="isEngLang">{{ model.name_en }}</span>
  ...
  <ul class="sub-menu" v-show="hasChildren">
    <side-menu-item
              v-for="model in model.menu_children_recursive"
              :model="model"
              :key="model.id">
    </side-menu-item>
  </ul>
...
export default {
  ...
  computed: {
    hasChildren() {
      return this.model.menu_children_recursive &&
        this.model.menu_children_recursive.length
    },
    currentUrl() {
